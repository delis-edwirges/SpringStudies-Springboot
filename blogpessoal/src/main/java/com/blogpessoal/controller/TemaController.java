...
public class TemaController {
	...
	@GetMapping ("/{id}")
	public ResponseEntity<TemaModel>  GetOId(@PathVariable long id){
	...
	public ResponseEntity<List<TemaModel>> GetByTema(@PathVariable String descricao){                   
		return ResponseEntity.ok(temaRepository.findAllByDescricaoContainingIgnoreCase(descricao));
	}
	
	@PostMapping
	public ResponseEntity<TemaModel> postarTema (@RequestBody TemaModel descricao){
		return ResponseEntity.status(HttpStatus.CREATED).body(temaRepository.save(descricao));
	
	}
	
	@PutMapping
	public ResponseEntity<TemaModel> putTema(@RequestBody TemaModel descricao){
